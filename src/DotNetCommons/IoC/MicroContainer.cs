...
public class MicroContainer : IDisposable
{
    ...
    protected Dictionary<string, object> Configuration { get; } = new(StringComparer.InvariantCultureIgnoreCase);
    protected readonly Dictionary<Type, Entry> Map = new();
    ...
    {
        var entry = Map.GetValueOrDefault(type);
        if (entry == null)
