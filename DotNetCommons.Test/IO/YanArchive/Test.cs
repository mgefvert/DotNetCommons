...
namespace DotNetCommons.Test.IO.YanArchive
{
    [TestClass]
    public class Test
    {
        ...
        public void RunTest(byte[] password)
        {
            using (var fs = new YanFileSystem("test.dat", password))
            {
                ...
                CollectionAssert.AreEqual(_testBytes, fs.Load("normal.html").ToArray());
                ...
                CollectionAssert.AreEqual(_testBytes, fs.Load("packed.html").ToArray());
